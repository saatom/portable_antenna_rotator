...
def IsEnabled():
    try:
        t1 = readMag()
        t2 = readAccl()
        print("3DSensor | Working")
        return True
    except:
        print("3DSensor | Nope, doesn't work")
        return False

# Output data to screen
#Accl = readAccl()
#Mag = readMag()
#print(f"Accelerometer | ({Accl[0]:.4f} X, {Accl[1]:.4f} Y, {Accl[2]:.4f} Z)")
#print(f"Magnetometer | ({Mag[0]:.4f} X, {Mag[1]:.4f} Y, {Mag[2]:.4f} Z)")
IsEnabled()